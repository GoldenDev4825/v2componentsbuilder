...
class V2ActionRowBuilder {
    ...
    setComponents(...components) {
        ...
        const isSelect = flattenedComponents[0].type !== v10_1.ComponentType.Button;
        if (isSelect) {
            if (flattenedComponents.length > 1)
                throw new Error("Action Rows can only contain 1 select component.");
        }
        else {
            if (flattenedComponents.some(c => c.type !== v10_1.ComponentType.Button)) {
    ...
    addComponents(...components) {
        ...
        if (newComponents.length === 0)
            return this;
        const isSelect = newComponents[0]?.type !== v10_1.ComponentType.Button;
        if (isSelect) {
            if (newComponents.length > 1)
                throw new Error("Action Rows can only contain 1 select component.");
        }
        else {
            if (newComponents.some(c => c.type !== v10_1.ComponentType.Button)) {
                throw new Error("Action Rows cannot mix buttons and select components.");
            }
